...
import { Sparkles, Users, Layers, CheckCircle2, ArrowRight } from 'lucide-react';
import { assembleTeam } from '../services/api';

export default function TeamAssembler({ availableSkills = [] }) {
  const [selectedSkills, setSelectedSkills] = useState(['FastAPI', 'React.js', 'Cypher & CognoDB']);
  ...
  const handleAssemble = async (targetSize = teamSize) => {
    ...
    setError(null);
    try {
      const data = await assembleTeam(selectedSkills, targetSize);
      setResult(data);
    } catch (err) {
        ...
        <div className="animate-fade-in">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
            <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#ffffff', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <Users size={22} color="var(--accent-cyan)" /> Recommended Team Combinations ({result.total_matches} Teams Assembled)
            </h3>
          </div>

          {result.recommended_partnerships?.length === 0 ? (
