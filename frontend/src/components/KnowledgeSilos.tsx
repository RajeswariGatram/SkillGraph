...
import { UserCheck, Building2, AlertTriangle } from 'lucide-react';
...
export default function KnowledgeSilos() {
  const [silos, setSilos] = useState<SiloItem[]>([]);
  ...
  useEffect(() => {
    async function load() {
      try {
        const data: any = await fetchKnowledgeSilos();
        setSilos(data.silos || data || []);
      } catch (err: any) {
            ...
            <div key={idx} className="glass-panel" style={{ padding: '20px', borderTop: '4px solid var(--accent-amber)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                ...
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <Building2 size={14} /> {item.department || 'Engineering'}
                </span>
              </div>

              <h4 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#ffffff', marginBottom: '8px' }}>
                {item.critical_skill || item.skill}
              </h4>
              <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '16px' }}>
                Category: <span style={{ color: 'var(--accent-cyan)' }}>{item.category || 'Core'}</span>
              </p>
              ...
              <div style={{ background: 'rgba(0,0,0,0.2)', padding: '12px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#ffffff' }}>{item.expert_name || item.employees?.[0] || 'Unknown Expert'}</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{item.expert_title || 'Specialist'}</div>
                  <div style={{ fontSize: '0.7rem', color: 'var(--accent-cyan)', marginTop: '2px' }}>{item.expert_email || 'expert@company.com'}</div>
                </div>
              </div>
            </div>
          ))}
